...
import com.hoanggiakiet.multi_thread_ex2.databinding.ActivityMain3Binding;
...
public class MainActivity3 extends AppCompatActivity {
    ...
    private void execLongRunningTask() {
        executorService.execute(new Runnable() {
            ...
            int width1 = 200;
            int width2 = binding.containerLayout.getWidth() - (binding.containerLayout.getPaddingLeft() + binding.containerLayout.getPaddingRight()) - width1 ;
            boolean rowOdd = true;
                ...
                for (int i = 1; i <= numberOfViews; i++) {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            int width, widthFull = width1 + width2;
                            ...
                            LinearLayout.LayoutParams paramsLy = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);

                            LinearLayout.LayoutParams paramsTv = new LinearLayout.LayoutParams(widthFull - width, 100);

                            paramsTv.setMargins(10, 20, 10, 20);
                            ...
                            if (binding.containerLayout.getChildCount() == 0) {
                                LinearLayout linearLayout = new LinearLayout(MainActivity3.this);
                                linearLayout.setLayoutParams(paramsLy);
                                binding.containerLayout.addView(linearLayout);
                            }

                            LinearLayout lastLY = (LinearLayout) binding.containerLayout.getChildAt(binding.containerLayout.getChildCount()-1);

                            if (lastLY.getChildCount() <= 2) {
                                lastLY.addView(textView);
                            } else {
                                LinearLayout linearLayout = new LinearLayout(MainActivity3.this);
                                linearLayout.setLayoutParams(paramsLy);
                    ...
                    try {
                        ...
                        Toast.makeText(MainActivity3.this, "Have error", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
